...
import { switchRole, logoutUser } from '../../redux/slices/authSlice';
    ...
    <header className="sticky top-0 z-30 flex h-18 w-full items-center justify-between border-b border-slate-200/90 bg-white/95 px-4 sm:px-6 lg:px-8 backdrop-blur-md shadow-2xs">
      ...
      <div className="flex items-center gap-3 sm:gap-4 flex-1">
        ...
        <button
          ...
          className="flex h-10 w-10 items-center justify-center rounded-xl border border-slate-200 text-slate-600 hover:bg-slate-50 lg:hidden cursor-pointer"
        >
          <Menu size={20} />
        </button>
      ...
      <div className="flex items-center gap-2.5 sm:gap-3.5">
        ...
        <button
          ...
          className={`inline-flex items-center gap-2 rounded-xl px-3.5 py-2 text-xs font-bold uppercase tracking-wider transition shadow-2xs cursor-pointer ${isCheckedIn
        ...
        {/* Chat / Messaging Workspace Button */}
        <Link
          to="/chat"
        ...
        <div className="relative">
          <button
            ...
            className="relative flex h-10 w-10 items-center justify-center rounded-xl border border-slate-200 text-slate-600 hover:bg-slate-50 hover:text-slate-900 transition cursor-pointer"
          >
            <Bell size={18} />
            {unreadCount > 0 && (
            ...
            <div className="absolute right-0 mt-2 w-80 sm:w-96 rounded-2xl border border-slate-200 bg-white p-4 shadow-xl z-50 animate-fadeIn">
              ...
              <div className="mt-3 space-y-2.5 max-h-80 overflow-y-auto">
                ...
                {notifications.map((item) => (
                  <div
                    key={item.id}
                    className={`rounded-xl p-3 text-xs transition border ${item.unread
        ...
        <div className="relative">
          <button
            ...
            className="flex items-center gap-2.5 rounded-xl border border-slate-200 p-1.5 hover:bg-slate-50 transition cursor-pointer"
          >
            <img
              src={userAvatar}
              alt={user?.name || 'User'}
              className="h-7 w-7 rounded-lg object-cover bg-slate-50"
            />
            <div className="hidden sm:flex flex-col text-left">
              <span className="text-xs font-bold text-slate-900 truncate max-w-[110px]">{user?.name}</span>
              <span className="text-[9px] font-mono font-bold text-blue-600 uppercase tracking-wider">{user?.role}</span>
            </div>
          ...
          {profileDropdownOpen && (
            ...
            >
              <div className="border-b border-slate-100 p-3">
                ...
                <span className="mt-1.5 inline-block rounded-full bg-blue-50 px-2 py-0.5 text-[9px] font-mono font-bold text-blue-700 uppercase">
                  {user?.role} PORTAL
                </span>
              </div>
